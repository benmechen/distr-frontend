...
const ServiceDetailsScreen = ({
    ...
    const handleCreate = async (input: Record<string, any>) => {
        const { data: response } = await createResource({
        ...
        if (!response?.resourceCreate.resource.id) return;
        next(
            response.resourceCreate.resource.id,
            response.resourceCreate.details,
        );
    };

    return (
        <LoadingWrapper loading={fetching || creating || fetchingInputs}>
